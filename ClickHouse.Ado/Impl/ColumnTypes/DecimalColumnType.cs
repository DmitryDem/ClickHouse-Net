...
namespace ClickHouse.Ado.Impl.ColumnTypes
{
    internal class DecimalColumnType : ColumnType
    {
        ...
        internal override async Task ReadAsync(ProtocolFormatter formatter, int rows)
        {
            ...
            var bytes = await formatter.ReadBytesAsync(rows * _byteLength).ConfigureAwait(false);
        ...
        public override async Task WriteAsync(ProtocolFormatter formatter, int rows)
        {
            ...
            foreach (var d in Data)
            {
                ...
                if (_byteLength == 4)
                {
                    await formatter.WriteBytesAsync(BitConverter.GetBytes((int)premultiplied)).ConfigureAwait(false);
                }
                else if (_byteLength == 8)
                {
                    await formatter.WriteBytesAsync(BitConverter.GetBytes((long)premultiplied)).ConfigureAwait(false);
                }
                else
                {
                    ...
                    {
                        await formatter.WriteByteAsync(i < bytes.Length ? bytes[i] : (byte)0).ConfigureAwait(false);
                    }
                }
            }
        }
